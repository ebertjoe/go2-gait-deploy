...
void LowLevelControl::run_policy()
{
    ...
    for (int i = 0; i < 12; i++)
    {
        ...
        cmd_msg_.motor_cmd[i].kp = 25.0;  // training gains (Isaac Lab Go2 actuator)
...
void LowLevelControl::state_obs()
{
    ...
    // 0.5, not 0.3: holding standing_angels_ with kp=50 leaves ~0.35 rad of
    // steady-state droop under gravity, so 0.3 never latches after a stand-up
    // from laydown and the policy state becomes unreachable. The laydown pose
    // is 2.57 away, so 0.5 still tells the two poses apart unambiguously.
